...
*** TODO admin artikel ohne vorlage markierung                      :feature:
    eventuell irgendwie mit raum verknüpfen?
*** TODO admin notifications                                        :feature:
...
*** TODO request: user soll genauer spezifizieren                   :feature:
...
**** ideen:
     ...
     - Räume durch Kategorien ersetzen (aussagekräftiger)
       - als eigenes Model implementieren:
         - name je nach sprache, 
         - bessere durchsuchbarkeit auf db ebene
         - ein artikel kann viele Kategorien haben
         - eine Kategorie kann Unterkategorien haben (Beispiel: Freizeit->Tauchen)
           - zur besseren Sortierung
     - Kategorie häufig verwendet: einfache Werkzeuge (in Alltagsgegenstände enthalten)
     - DONE kategorien erst mal einklappen
     - absolute sparvariante:
       keine artikeldatenbank, sondern im profil ein textfeld "was verleihst du?", das durchsucht wird
       netter gedanke, aber nicht so hilfreich
     - Inventar und Vorlagen Seite vereinen zu "Verleihen"
     ...
     - Räume haben noch keine Übersetzungen
